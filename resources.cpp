...
RM_Textures::RM_Textures()
{
    ...
    std::string path = "resources/graphics/";
    ...
    storage.emplace(TEXTURES::POWERFUL_WARRIOR, path + "powerful_warrior.png"s);
    storage.emplace(TEXTURES::CASTER, path + "caster.png"s);
    storage.emplace(TEXTURES::VETERAN_CASTER, path + "veteran_caster.png"s);
    storage.emplace(TEXTURES::GOLEM, path + "golem.png"s);
    storage.emplace(TEXTURES::RESISTANT_GOLEM, path + "resistant_golem.png"s);
    storage.emplace(TEXTURES::ELEMENTAL, path + "elemental.png"s);
    storage.emplace(TEXTURES::ELDER_ELEMENTAL, path + "elder_elemental.png"s);
...
RM_Shaders::RM_Shaders()
{
    ...
    storage.emplace(SHADERS::LIGHTS, std::make_pair(path + "lights.vert"s, path + "lights.frag"s));
}

sf::Shader& RM_Shaders::load_resource(std::pair<std::string, std::string> files, RESOURCE_ID::SHADERS id)
{
    auto shader = std::make_unique<sf::Shader>();
    shader->loadFromFile(files.first, files.second);

    auto res = cache.emplace(id, std::move(shader));
    return *res.first->second;
}