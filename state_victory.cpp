...
box(arg_app_handle.get_shared_context().resources, arg_app_handle.get_shared_context().sfx_manager)
{
	...
	if (casted_pack.points > casted_pack.lp.achieved_points)
	{
		...
		if (casted_pack.lp.campaign_index > 0)
		{
			auto player_data = load_player_data(app_handle.get_shared_context().config_manager.player_data_path);
			new_cells = casted_pack.points - previous_points;
			player_data.cells_balance += new_cells;
			player_data.total_cells += new_cells;
	...
	if (casted_pack.lp.campaign_index == 0)
		...
		box.set_text(hct::dynarray<std::string>({
			...
			make_string("Points Gathered: ", casted_pack.points),
			make_string("Previous Best: ", previous_points),
			(new_cells > 0 ? make_string("You got ", new_cells, " new Power Cells!") : "You did not get any new Power Cells.") }));
