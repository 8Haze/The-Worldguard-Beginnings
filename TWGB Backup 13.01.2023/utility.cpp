...
void set_stats_by_level(Player_Data& pd)
{
	switch (pd.level)
	{
	case(1): pd.health = 12; pd.damage = 4; pd.cells_for_next_upgrade = 200'000; break;
	case(2): pd.health = 15; pd.damage = 4; pd.cells_for_next_upgrade = 200'000; break;
	case(3): pd.health = 18; pd.damage = 6; pd.cells_for_next_upgrade = 200'000; break;
	case(4): pd.health = 21; pd.damage = 6; pd.cells_for_next_upgrade = 400'000; break;
	case(5): pd.health = 24; pd.damage = 8; pd.cells_for_next_upgrade = 0; break;
	default: throw std::runtime_error("load_player_data: File corrupted");
	}
}

Player_Data load_player_data(const std::filesystem::path& path)
{
	Player_Data pd;
	std::ifstream file(path, std::ios::binary);
	auto guard = hct::make_guard_exit([&file]() { file.close(); });

	data_reader(file, pd.level);
	data_reader(file, pd.cells_balance);
	data_reader(file, pd.total_cells);

	set_stats_by_level(pd);

	return pd;
}

void save_player_data(Player_Data pd, const std::filesystem::path& path)
{
	std::ofstream file(path, std::ios::binary);
	auto guard = hct::make_guard_exit([&file]() { file.close(); });
	data_writer(file, pd.level);
	data_writer(file, pd.cells_balance);
	data_writer(file, pd.total_cells);
}
