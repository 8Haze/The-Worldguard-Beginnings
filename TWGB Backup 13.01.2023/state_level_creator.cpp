...
void State_Level_Creator::update_option_info()
{
	...
	case(TOOLBELT_OPTIONS::LIGHTS): option_info.setString("Lights"); break;
...
void State_Level_Creator::handle_toolbelt_events(const sf::Event& event)
{
	...
	case (TOOLBELT_OPTIONS::LIGHTS):
	{
		app_handle.request_push(App::STATE::LEVEL_CREATOR_OBJECT_SELECTION_LIGHT);
		break;
	}
...
void State_Level_Creator::remove_objects(int32_t x, int32_t y)
{
	...
	light_manager.remove_light(sf::Vector2f(x, y));
...
void State_Level_Creator::render_lights()
{
	...
	light_manager.draw(
		...
		static_cast<int>(camera.getCenter().y - camera.getSize().y * 0.5f - 64.f),
		camera.getCenter().x * 0.5f - render_texture.getView().getCenter().x * 0.5f,
		camera.getCenter().y * 0.5f - render_texture.getView().getCenter().y * 0.5f,
		false);
...
void State_Level_Creator::save_data()
{
...

	{
		...
		light_manager.record_lights(ofs);
	}
