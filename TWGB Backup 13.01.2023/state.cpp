...
SCPP_LCOS_Light::SCPP_LCOS_Light(int32_t arg_index) :
	SCPP_LCOS_Base(LO_TYPE::LIGHT_SOURCE), index(arg_index)
{
	info[0] += "Light Source";
	info[1] = "Color: ";
	info[1] += index_to_light_color_name(index);
}

void SCPP_LCOS_Light::record_lo_subtype(std::ostream& stream)
{
	data_writer(stream, static_cast<int32_t>(0));
}

void SCPP_LCOS_Light::record_extra_data(std::ostream& stream)
{
	data_writer(stream, index);
}
