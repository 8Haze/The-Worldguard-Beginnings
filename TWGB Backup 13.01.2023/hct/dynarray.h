...
namespace hct
{
	...
	public:
		...
		dynarray() noexcept(std::is_nothrow_default_constructible_v<Allocator>) : allocator(Allocator{}) {}

		explicit dynarray(const allocator_type& arg_allocator) noexcept : allocator(arg_allocator) {}

		explicit dynarray(size_type arg_n, const value_type& arg_value, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
			resize(arg_n, arg_value);
		}

		explicit dynarray(size_type arg_n, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
			resize(arg_n);
		}

		template <typename Iter, std::enable_if_t<std::input_iterator<Iter>, bool> = true>
		explicit dynarray(Iter arg_iBegin, Iter arg_iEnd, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
		...
		dynarray(const dynarray& arg_other, const allocator_type& arg_allocator) : allocator(arg_allocator)
		{
			reserve(arg_other.data_size);
			for (const auto& x : arg_other)
				push_back(x);
		...
		dynarray(dynarray&& arg_other) noexcept : allocator(std::move(arg_other.allocator)),
			data(arg_other.data), idx_end(arg_other.idx_end), data_size(arg_other.data_size)
		{
			arg_other.reset();
		}
		...
		dynarray(std::initializer_list<T> arg_il, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
			reserve(arg_il.size());
			try
			{
