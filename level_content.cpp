...
Level_Content::Level_Content(App& arg_app_handle) :
	...
	light_manager(render_texture, arg_app_handle.get_shared_context().resources.shaders),
...
void Level_Content::register_objects()
{
	...
	interactive_tile_factory_registry.emplace(INTERACTIVE_TILE_SUBTYPE::LIGHT, Light_Tile::make_lo);
	interactive_tile_factory_registry.emplace(INTERACTIVE_TILE_SUBTYPE::DARK, Dark_Tile::make_lo);
	...
	colliding_entity_factory_registry.emplace(COLLIDING_ENTITY_SUBTYPE::VETERAN_CASTER, Veteran_Caster::make_lo);
	colliding_entity_factory_registry.emplace(COLLIDING_ENTITY_SUBTYPE::ELEMENTAL, Elemental::make_lo);
	colliding_entity_factory_registry.emplace(COLLIDING_ENTITY_SUBTYPE::ELDER_ELEMENTAL, Elder_Elemental::make_lo);
	...
	walking_entity_factory_registry.emplace(WALKING_ENTITY_SUBTYPE::POWERFUL_WARRIOR, Powerful_Warrior::make_lo);
	walking_entity_factory_registry.emplace(WALKING_ENTITY_SUBTYPE::GOLEM, Golem::make_lo);
	walking_entity_factory_registry.emplace(WALKING_ENTITY_SUBTYPE::RESISTANT_GOLEM, Resistant_Golem::make_lo);
...
void Level_Content::create_new_object(App& app_handle, std::istream& source_stream, int32_t type, int32_t subtype)
{
	...
	case(LO_TYPE::WALKING_ENTITY):
		...
		break;
	case(LO_TYPE::LIGHT_SOURCE):
		light_manager.read_light(source_stream);
		break;
