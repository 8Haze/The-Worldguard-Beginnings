...
void Light_Tile::set_desired_sprite_details()
{
	set_sprite_rect({ 32 * 8, 0, 32, 32 });
}

void Light_Tile::extra_pre_record(std::ostream& stream) const
{
	data_writer(stream, static_cast<int32_t>(LO_TYPE::INTERACTIVE_TILE));
	data_writer(stream, static_cast<int32_t>(INTERACTIVE_TILE_SUBTYPE::LIGHT));
}

sf::FloatRect Light_Tile::get_hitbox() const noexcept
{
	return ::get_hitbox(get_sprite(), 0.5f, 0.7f);
}

void Light_Tile::player_interaction(Player& player)
{
	if (collision(get_hitbox(), player.get_hitbox()))
		player.set_visible_light(1.f);
}

Interactive_Tile* Light_Tile::make_lo()
{
	return new Light_Tile;
}
...
void Dark_Tile::set_desired_sprite_details()
{
	set_sprite_rect({ 32 * 7, 0, 32, 32 });
}

void Dark_Tile::extra_pre_record(std::ostream& stream) const
{
	data_writer(stream, static_cast<int32_t>(LO_TYPE::INTERACTIVE_TILE));
	data_writer(stream, static_cast<int32_t>(INTERACTIVE_TILE_SUBTYPE::DARK));
}

sf::FloatRect Dark_Tile::get_hitbox() const noexcept
{
	return ::get_hitbox(get_sprite(), 0.5f, 0.7f);
}

void Dark_Tile::player_interaction(Player& player)
{
	if (collision(get_hitbox(), player.get_hitbox()))
		player.set_visible_light(0.6f);
}

Interactive_Tile* Dark_Tile::make_lo()
{
	return new Dark_Tile;
}
...
{
	set_sprite_rect({ 32 * 12 + (moves_left ? 0 : 32), 0, 32, 32 });
}
