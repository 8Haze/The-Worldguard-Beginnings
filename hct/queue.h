...
namespace hct
{
	...
	public:
		...
		queue(const queue& arg_other) : container(arg_other.container) {}

		queue(queue&& arg_other) noexcept : container(std::move(arg_other.container)) {}

		explicit queue(const Container& arg_container) : container(arg_container) {}

		explicit queue(Container&& arg_container) noexcept(std::is_nothrow_move_constructible_v<Container>) : container(std::move(arg_container)) {}
		...
		explicit queue(const Alloc& arg_allocator) noexcept(std::is_nothrow_constructible_v<Container, const Alloc&>) : container(arg_allocator) {}

		template <class Alloc, std::enable_if_t<std::uses_allocator_v<Container, Alloc>, int> = 0>
		queue(const Container& arg_container, const Alloc& arg_allocator) : container(arg_container, arg_allocator) {}

		template <class Alloc, std::enable_if_t<std::uses_allocator_v<Container, Alloc>, int> = 0>
		queue(Container&& arg_container, const Alloc& arg_allocator) noexcept(std::is_nothrow_constructible_v<Container, Container, const Alloc&>) : container(arg_container, arg_allocator) {}

		template <class Alloc, std::enable_if_t<std::uses_allocator_v<Container, Alloc>, int> = 0>
		queue(const queue& arg_other, const Alloc& arg_allocator) : container(arg_other.container, arg_allocator) {}

		template <class Alloc, std::enable_if_t<std::uses_allocator_v<Container, Alloc>, int> = 0>
		queue(queue&& arg_other, const Alloc& arg_allocator) noexcept(std::is_nothrow_constructible_v<Container, Container, const Alloc&>) : container(std::move(arg_other.container), arg_allocator) {}

		template <class Iter, class Alloc, std::enable_if_t<std::uses_allocator_v<Container, Alloc>, int> = 0>
		queue(Iter arg_iBegin, Iter arg_iEnd, const Alloc& arg_allocator) : container(arg_allocator)
		{
			for (; arg_iBegin != arg_iEnd; ++arg_iBegin)
				container.push_back(*arg_iBegin);
		}
