...
namespace hct
{
	...
	public:
		...
		deck_const_iterator(data_pointer arg_ptr, data_pointer arg_lim_first, data_pointer arg_lim_last, bool arg_switched) :
			ptr(arg_ptr), lim_first(arg_lim_first), lim_last(arg_lim_last), switched(arg_switched) {}
	...
	public:
		...
		deck() noexcept(std::is_nothrow_default_constructible_v<Allocator>) : allocator(Allocator{}) {}

		explicit deck(const allocator_type& arg_allocator) noexcept : allocator(arg_allocator) {}

		explicit deck(size_type arg_n, const value_type& arg_value, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
			resize(arg_n, arg_value);
		}

		explicit deck(size_type arg_n, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
			resize(arg_n);
		}

		template <typename Iter, std::enable_if_t<std::input_iterator<Iter>, bool> = true>
		explicit deck(Iter arg_iBegin, Iter arg_iEnd, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
		...
		deck(const deck& arg_other, const allocator_type& arg_allocator) : allocator(arg_allocator)
		{
			reserve(arg_other.data_size);
			for (const auto& x : arg_other)
				push_back(x);
		...
		deck(deck&& arg_other) noexcept : allocator(std::move(arg_other.allocator)), data(arg_other.data),
			idx_start(arg_other.idx_start), idx_end(arg_other.idx_end), data_size(arg_other.data_size), count(arg_other.count)
		{
			arg_other.reset();
		}
		...
		deck(std::initializer_list<T> arg_il, const allocator_type& arg_allocator = Allocator{}) : allocator(arg_allocator)
		{
			reserve(arg_il.size());
			try
			{
