...
Caster_Base::Caster_Base(LEVEL_THEME theme_id, unsigned int health, unsigned int damage, int points) :
	Bullet_Manager_Component(theme_to_bullet_color(theme_id), false), Enemy_Component(health, damage, points)
{
...
void Caster_Base::set_death_graphics(Resources& resources)
{
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::EXPLOSION_48));
	set_sprite_animation(get_animation(AID_MISC::EXPLOSION_48));
}

void Caster_Base::extra_player_interaction(Resources& resources, Player& player, const sf::FloatRect& my_hitbox)
{
...
sf::FloatRect Caster_Base::get_hitbox()
{
	return ::get_hitbox(get_sprite(), 0.8f, 0.8f);
}

void Caster_Base::perform_update(Resources& resources, Player& player, const sf::Time& dt, bool shoot_ready,
	const hct::dynarray<std::unique_ptr<Solid_Tile>>& solid_tiles,
	const hct::dynarray<std::unique_ptr<Destructible_Tile>>& destructible_tiles)
{
	update_frame(dt);
...
void Caster_Base::draw(sf::RenderTexture& rt) const
{
	draw_subentities(rt);
	draw_health_info(rt);
	this->LO_Base::draw(rt);
...
Caster::Caster(LEVEL_THEME theme_id) :
	Caster_Base(theme_id, health_stat, damage_stat, colliding_entity_value(COLLIDING_ENTITY_SUBTYPE::CASTER)) {}
...
void Caster::update(Resources& resources, Player& player, const sf::Time& dt,
	...
	const hct::dynarray<std::unique_ptr<Destructible_Tile>>& destructible_tiles)
{
	perform_update(resources, player, dt, shoot_timer.is_ready(), solid_tiles, destructible_tiles);
}
...
Veteran_Caster::Veteran_Caster(LEVEL_THEME theme_id) :
	Caster_Base(theme_id, health_stat, damage_stat, colliding_entity_value(COLLIDING_ENTITY_SUBTYPE::VETERAN_CASTER)) {}

void Veteran_Caster::extra_setup(std::istream& stream, Resources& resources)
{
	setup_health_info(resources);
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::VETERAN_CASTER));
	center_sprite();
}

void Veteran_Caster::extra_pre_record(std::ostream& stream) const
{
	data_writer(stream, static_cast<int32_t>(LO_TYPE::COLLIDING_ENTITY));
	data_writer(stream, static_cast<int32_t>(COLLIDING_ENTITY_SUBTYPE::VETERAN_CASTER));
}

void Veteran_Caster::update(Resources& resources, Player& player, const sf::Time& dt,
	const hct::dynarray<std::unique_ptr<Solid_Tile>>& solid_tiles,
	const hct::dynarray<std::unique_ptr<Destructible_Tile>>& destructible_tiles)
{
	perform_update(resources, player, dt, shoot_timer.is_ready(), solid_tiles, destructible_tiles);
...
void Veteran_Caster::update_timer(const sf::Time& dt)
{
	shoot_timer.update(dt);
}

Colliding_Entity* Veteran_Caster::make_lo(LEVEL_THEME theme_id)
{
	return new Veteran_Caster(theme_id);
}
...
Elemental_Base::Elemental_Base(LEVEL_THEME theme_id, unsigned int health, unsigned int damage, int points, float shoot_countdown_time) :
	Bullet_Manager_Component(theme_to_bullet_color(theme_id), false), Enemy_Component(health, damage, points), shoot_countdown(shoot_countdown_time)
{
...
void Elemental_Base::set_death_graphics(Resources& resources)
{
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::EXPLOSION_32));
	set_sprite_animation(get_animation(AID_MISC::EXPLOSION_32));
}

void Elemental_Base::extra_player_interaction(Resources& resources, Player& player, const sf::FloatRect& my_hitbox)
{
	if (player.get_position().x < get_sprite().getPosition().x)
		get_sprite_animation().horizontally_flipped = true;
	else if (player.get_position().x > get_sprite().getPosition().x)
...
sf::FloatRect Elemental_Base::get_hitbox()
{
	return ::get_hitbox(get_sprite(), 0.9f, 0.9f);
}

void Elemental_Base::perform_update(Resources& resources, Player& player, const sf::Time& dt, float speed,
	const hct::dynarray<std::unique_ptr<Solid_Tile>>& solid_tiles,
	const hct::dynarray<std::unique_ptr<Destructible_Tile>>& destructible_tiles)
{
	update_frame(dt);
...
void Elemental_Base::draw(sf::RenderTexture& rt) const
{
	draw_subentities(rt);
	draw_health_info(rt);
	this->LO_Base::draw(rt);
...
Elemental::Elemental(LEVEL_THEME theme_id) :
	Elemental_Base(theme_id, health_stat, damage_stat, colliding_entity_value(COLLIDING_ENTITY_SUBTYPE::ELEMENTAL), shoot_countdown) {}
...
void Elemental::update(Resources& resources, Player& player, const sf::Time& dt,
	...
	const hct::dynarray<std::unique_ptr<Destructible_Tile>>& destructible_tiles)
{
	Elemental::perform_update(resources, player, dt, speed_stat, solid_tiles, destructible_tiles);
}
...
Elder_Elemental::Elder_Elemental(LEVEL_THEME theme_id) :
	Elemental_Base(theme_id, health_stat, damage_stat, colliding_entity_value(COLLIDING_ENTITY_SUBTYPE::ELDER_ELEMENTAL), shoot_countdown) {}

void Elder_Elemental::extra_setup(std::istream& stream, Resources& resources)
{
	setup_health_info(resources);
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::ELDER_ELEMENTAL));
	center_sprite();
}

void Elder_Elemental::extra_pre_record(std::ostream& stream) const
{
	data_writer(stream, static_cast<int32_t>(LO_TYPE::COLLIDING_ENTITY));
	data_writer(stream, static_cast<int32_t>(COLLIDING_ENTITY_SUBTYPE::ELDER_ELEMENTAL));
}

void Elder_Elemental::update(Resources& resources, Player& player, const sf::Time& dt,
	const hct::dynarray<std::unique_ptr<Solid_Tile>>& solid_tiles,
	const hct::dynarray<std::unique_ptr<Destructible_Tile>>& destructible_tiles)
{
	perform_update(resources, player, dt, speed_stat, solid_tiles, destructible_tiles);
}

Colliding_Entity* Elder_Elemental::make_lo(LEVEL_THEME theme_id)
{
	return new Elder_Elemental(theme_id);
}
...
void Warrior_Base::set_death_graphics(Resources& resources)
{
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::EXPLOSION_48));
	set_sprite_animation(get_animation(AID_MISC::EXPLOSION_48));
}

Warrior_Base::Warrior_Base(LEVEL_THEME theme_id, unsigned int health, unsigned int damage, int points) :
	Enemy_Component(health, damage, points)
{
...
sf::FloatRect Warrior_Base::get_hitbox()
{
	return ::get_hitbox(get_sprite(), 0.8f, 0.8f);
}

void Warrior_Base::perform_update(Resources& resources, Player& player, const sf::Time& dt, float speed)
{
	update_frame(dt);

	if (is_alive())
	{
		update_walking(dt, speed);
...
void Warrior_Base::draw(sf::RenderTexture& rt) const
{
	draw_health_info(rt);
	this->LO_Base::draw(rt);
}
...
Warrior::Warrior(LEVEL_THEME theme_id) :
	Warrior_Base(theme_id, health_stat, damage_stat, walking_entity_value(WALKING_ENTITY_SUBTYPE::WARRIOR)) {}
...
{
	perform_update(resources, player, dt, speed_stat);
}
...
Powerful_Warrior::Powerful_Warrior(LEVEL_THEME theme_id) :
	Warrior_Base(theme_id, health_stat, damage_stat, walking_entity_value(WALKING_ENTITY_SUBTYPE::POWERFUL_WARRIOR)) {}

void Powerful_Warrior::extra_setup(std::istream& stream, Resources& resources)
{
	setup_health_info(resources);
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::POWERFUL_WARRIOR));
}

void Powerful_Warrior::extra_pre_record(std::ostream& stream) const
{
	data_writer(stream, static_cast<int32_t>(LO_TYPE::WALKING_ENTITY));
	data_writer(stream, static_cast<int32_t>(WALKING_ENTITY_SUBTYPE::POWERFUL_WARRIOR));
}

void Powerful_Warrior::update(Resources& resources, Player& player, const sf::Time& dt)
{
	perform_update(resources, player, dt, speed_stat);
}

Walking_Entity* Powerful_Warrior::make_lo(LEVEL_THEME theme_id)
{
	return new Powerful_Warrior(theme_id);
}
...
Golem_Base::Golem_Base(LEVEL_THEME theme_id, unsigned int health, unsigned int damage, int points) :
	Enemy_Component(health, damage, points)
{
	auto animation = get_animation(AID_ENTITY::GOLEMS);
	animation.row = static_cast<int>(theme_id) - 1;
	if (animation.row < 0 || theme_id >= LEVEL_THEME::COUNT)
		throw std::runtime_error("Golem_Base::Golem_Base: invalid theme_id provided");
	set_sprite_animation(animation);
}

void Golem_Base::set_death_graphics(Resources& resources)
{
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::EXPLOSION_48));
	set_sprite_animation(get_animation(AID_MISC::EXPLOSION_48));
}

sf::FloatRect Golem_Base::get_hitbox()
{
	return ::get_hitbox(get_sprite(), 0.7f, 0.8f);
}

void Golem_Base::perform_update(Resources& resources, Player& player, const sf::Time& dt, float speed, unsigned int regeneration_value)
{
	update_frame(dt);
	...
	if (is_alive())
	{
		...
		if (is_alive())
		{
			if (regeneration_timer.is_ready())
				receive_heal(regeneration_value);
...
void Golem_Base::draw(sf::RenderTexture& rt) const
{
	draw_health_info(rt);
	this->LO_Base::draw(rt);
}
...
Golem::Golem(LEVEL_THEME theme_id) :
	Golem_Base(theme_id, health_stat, damage_stat, walking_entity_value(WALKING_ENTITY_SUBTYPE::GOLEM)) {}
...
{
	perform_update(resources, player, dt, speed_stat, regeneration_stat);
}
...
Resistant_Golem::Resistant_Golem(LEVEL_THEME theme_id) :
	Golem_Base(theme_id, health_stat, damage_stat, walking_entity_value(WALKING_ENTITY_SUBTYPE::RESISTANT_GOLEM)) {}

void Resistant_Golem::extra_setup(std::istream& stream, Resources& resources)
{
	setup_health_info(resources);
	set_sprite_texture(resources.textures.get(RESOURCE_ID::TEXTURES::RESISTANT_GOLEM));
}

void Resistant_Golem::extra_pre_record(std::ostream& stream) const
{
	data_writer(stream, static_cast<int32_t>(LO_TYPE::WALKING_ENTITY));
	data_writer(stream, static_cast<int32_t>(WALKING_ENTITY_SUBTYPE::RESISTANT_GOLEM));
}

void Resistant_Golem::update(Resources& resources, Player& player, const sf::Time& dt)
{
	perform_update(resources, player, dt, speed_stat, regeneration_stat);
}

Walking_Entity* Resistant_Golem::make_lo(LEVEL_THEME theme_id)
{
	return new Resistant_Golem(theme_id);
}
