	...
	box(arg_app_handle.get_shared_context().resources, arg_app_handle.get_shared_context().sfx_manager)
{
	...
	auto player = load_player_data(Config_Manager::player_data_path);
	player.level = 1;
	player.cells_balance = 0;
	player.total_cells = 0;
	save_player_data(player, Config_Manager::player_data_path);
